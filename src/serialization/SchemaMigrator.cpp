...
Expected<std::string, ParseError> SchemaMigrator::migrate(const std::string &jsonText) {
    ...
    if (it == data.end() || !it->is_number_integer()) {
        return Unexpected(ParseError("missing a valid schemaVersion field"));
    }
    const int fromVersion = it->get<int>();
    if (fromVersion < 1 || fromVersion > currentVersion()) {
        return Unexpected(ParseError("unsupported schemaVersion: " + std::to_string(fromVersion)));
    }
