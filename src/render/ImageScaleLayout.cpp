...
ImageRect ComputeImageDestinationRect(Vec2 containerSize, Vec2 intrinsicSize,
    ...
    switch (mode) {
        ...
        case ImageScaleMode::LetterBox: {
            const float scale =
                std::min(containerSize.x / intrinsicSize.x, containerSize.y / intrinsicSize.y);
            const float width = intrinsicSize.x * scale;
            const float height = intrinsicSize.y * scale;
            return ImageRect{(containerSize.x - width) * 0.5f, (containerSize.y - height) * 0.5f,
                             width, height};
        }
        case ImageScaleMode::Zoom: {
            const float scale =
                std::max(containerSize.x / intrinsicSize.x, containerSize.y / intrinsicSize.y);
            const float width = intrinsicSize.x * scale;
            const float height = intrinsicSize.y * scale;
            return ImageRect{(containerSize.x - width) * 0.5f, (containerSize.y - height) * 0.5f,
                             width, height};
