...
T Animatable<T>::evaluate(FrameTime time) const {
    return evaluatePreview(static_cast<PreviewTime>(time));
}
...
T Animatable<T>::evaluatePreview(PreviewTime time) const {
    ...
    if (time <= static_cast<PreviewTime>(keyframes_.front().time)) {
        return keyframes_.front().value;
    }
    if (time >= static_cast<PreviewTime>(keyframes_.back().time)) {
...
template <>
AnimatableType Animatable<VectorNetwork>::valueType() const {
    return AnimatableType::VectorNetwork;
}
template <>
...
template <>
AnimatableType Animatable<Vec4>::valueType() const {
    return AnimatableType::Vec4;
}
