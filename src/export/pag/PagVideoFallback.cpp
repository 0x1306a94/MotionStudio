...
void ApplyHostScale(pag::PreComposeLayer *pagLayer, int hostWidth, int hostHeight, int bitmapWidth,
                    int bitmapHeight) {
    if (pagLayer == nullptr || pagLayer->transform == nullptr || bitmapWidth <= 0 ||
        bitmapHeight <= 0) {
        return;
    }
    const float scaleX = static_cast<float>(hostWidth) / static_cast<float>(bitmapWidth);
    const float scaleY = static_cast<float>(hostHeight) / static_cast<float>(bitmapHeight);
    if (scaleX == 1.0f && scaleY == 1.0f) {
        return;
    }
    delete pagLayer->transform->scale;
    pagLayer->transform->scale = new pag::Property<pag::Point>(pag::Point::Make(scaleX, scaleY));
}
...
Expected<VideoFallbackResult, PagExportError> PagVideoFallback::Build(
    ...
    if (duration <= 0) {
        ...
        return Unexpected(MakePagExportError(
            ...
            "Layer \"" + name + "\": visible range is empty; cannot export as video"));
    }
    ...
    if (!prepared.hasValue()) {
        ...
        return Unexpected(MakePagExportError(
            PagExportErrorKind::MappingFailed, rootLayer.id, name, "BitmapForcedByLayerName",
            "Layer \"" + name + "\": video FrameSource prepare failed: " + prepared.error()));
    }

    auto *videoComposition = new pag::VideoComposition();
    videoComposition->id = compositionId;
    videoComposition->width = size.width;
    videoComposition->height = size.height;
    videoComposition->duration = duration;
    videoComposition->frameRate = static_cast<float>(hostComposition.frameRate.num) /
        static_cast<float>(hostComposition.frameRate.den);

    auto *sequence = new pag::VideoSequence();
    sequence->composition = videoComposition;
    ...
    ApplyHostScale(pagLayer, hostComposition.width, hostComposition.height, size.width,
                   size.height);
    pagLayer->composition = videoComposition;
    pagLayer->compositionStartTime = 0;

    PushWarning(warnings, rootLayer.id, "BitmapForcedByLayerName",
                "Layer name ends with _bmp; exported as VideoComposition");
    if (rootLayer.type() == LayerType::Group) {
        PushWarning(warnings, rootLayer.id, "GroupSubtreeRasterized",
                    "Group subtree rasterized into one VideoComposition");
    }

    VideoFallbackResult result;
    result.layer = pagLayer;
    result.composition = videoComposition;
    return result;
}
...
Expected<pag::VideoComposition *, PagExportError> PagVideoFallback::BuildComposition(
    ...
    if (composition.duration <= 0) {
        ...
        return Unexpected(MakePagExportError(
            ...
            "Composition \"" + name + "\": duration is empty; cannot export as video"));
    }
    ...
    if (!prepared.hasValue()) {
        ...
        return Unexpected(MakePagExportError(
            PagExportErrorKind::MappingFailed, composition.id, name, "BitmapForcedByCompositionName",
            "Composition \"" + name +
                "\": video FrameSource prepareComposition failed: " + prepared.error()));
    }
    ...
    videoComposition->width = size.width;
    videoComposition->height = size.height;
    videoComposition->duration = composition.duration;
    videoComposition->frameRate = static_cast<float>(composition.frameRate.num) /
        static_cast<float>(composition.frameRate.den);
    videoComposition->backgroundColor = ToPagColor(composition.backgroundColor);
